...
data_root='./smpcupData/'
post_data = pd.read_table(data_root+'data/2_Post.txt' ,sep='\001' ,names=['userID' ,'blogID' ,'date'])
browse_data = pd.read_table(data_root+'data/3_Browse.txt' ,sep='\001' ,names=['userID' ,'blogID' ,'date'])
comment_data = pd.read_table(data_root+'data/4_Comment.txt' ,sep='\001' ,names=['userID' ,'blogID' ,'date'])
voteup_data = pd.read_table(data_root+'data/5_Vote-up.txt' ,sep='\001' ,names=['userID' ,'blogID' ,'date'])
votedown_data = pd.read_table(data_root+'data/6_Vote-down.txt' ,sep='\001' ,names=['userID' ,'blogID' ,'date'])
favorite_data = pd.read_table(data_root+'data/7_Favorite.txt' ,sep='\001' ,names=['userID' ,'blogID' ,'date'])
follow_data = pd.read_table(data_root+'data/8_Follow.txt' ,sep='\001' ,names=['userID1' ,'userID2'])
letter_data = pd.read_table(data_root+'data/9_Letter.txt' ,sep='\001' ,names=['userID1' ,'userID2' ,'date'])
...
secondActSumTimeM = commentTimeM.add(secondActSumTimeM, fill_value=0) + \
                    voteupTimeM.add(secondActSumTimeM, fill_value=0) + votedownTimeM.add(secondActSumTimeM,
                                                                                         fill_value=0) \
...
secondActSumTimePre = commentTimePre.add(secondActSumTimePre, fill_value=0) + \
                      voteupTimePre.add(secondActSumTimePre, fill_value=0) + votedownTimePre.add(secondActSumTimePre,
                                                                                                 fill_value=0) \
...
stadata = pd.read_csv(data_root+'data/actStatisticData.csv').sort_index()
...
stadata_new.to_csv(data_root+'data/actStatisticData_new1.csv' ,index=False)
